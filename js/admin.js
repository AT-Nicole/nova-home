...
function updateNavCounts() {
  ...
  $("#nav-draft-count").textContent = loadDrafts().length;
...
function renderSettings() {
  ...
  const el = $("#view-content");
  el.innerHTML =
    '<div class="adm-card"><h2>公司信息与联系方式</h2><div class="card-sub">以下内容会同步到全站页眉、页脚、联系页与 WhatsApp 链接。</div>' +
...
function renderInquiries() {
  ...
  if (!list.length) {
    el.innerHTML = '<div class="adm-card" style="text-align:center;color:var(--ink-3);padding:60px 20px;">暂无询盘。前台表单提交后会显示在这里（注意：localStorage 数据与浏览器绑定，正式服务器请接 api/submit.php 并改用数据库）。</div>';
    return;
  }
  ...
  $$("[data-del]", el).forEach(function (b) {
    b.addEventListener("click", function () {
      ...
      store.saveInquiries(list.filter(function (x) { return x.id !== id; }));
      renderInquiries();
      updateNavCounts();
    });
  });
...
const AI_SPEC_KEYS = ["power", "voltage", "frequency", "size", "weight", "material", "warranty", "moq", "colors", "control", "noise", "heating", "airflow", "speeds", "battery", "runtime", "waterproof", "certification", "blades", "timer", "levels", "dimension", "netweight", "grossweight", "packsize", "loading", "display", "cookware", "oscillation", "remote", "ion", "voltageRange"];
const AI_CATS = ["induction-cooker", "hair-dryer", "fan", "heater", "razor"];
...
const AI_PROMPT = [
  ...
  "{",
  '  "name_en": "product name in English, include model number if shown",',
  '  "cat": one of: "induction-cooker", "hair-dryer", "fan", "heater", "razor", "other",',
  '  "specs": [{"k": "...", "v": "..."}],  // k must be one of the allowed keys below; skip specs that do not fit',
  '  "desc_en": "one short English marketing sentence (model, key features)"',
  "}",
  "Allowed spec keys (exact match): power, voltage, frequency, size, weight, material, warranty, moq, colors, control, noise, heating, airflow, speeds, battery, runtime, waterproof, certification, blades, timer, levels, dimension, netweight, grossweight, packsize, loading, display, cookware, oscillation, remote, ion, voltageRange",
  "Rules: no markdown, no explanation, ONLY the JSON array. If no product appears on this page, return [].",
...
VIEWS.aiparse = { title: "AI 产品录入", sub: "上传产品目录 PDF，AI 自动提取产品生成草稿，审核后一键上架（API Key 在「网站设置」里配置）", render: renderAiParse };
...
function renderAiParse() {
  ...
  el.innerHTML =
    ...
    '<div class="card-sub">支持扫描版 / 图片版目录：AI 把每一页转成图片后用视觉模型识别。建议一次传一个品类，识别更准；页数多时耐心等进度条走完。</div>' +
  ...
  ensurePdfJs(function () {
    (async function () {
      try {
        ...
        saveDrafts(drafts.concat(items));
        setAiProgress(100, "完成！识别到 " + items.length + " 个产品，已生成草稿");
        toast("解析完成：" + items.length + " 个产品草稿待审核");
        renderAiParse();
        updateNavCounts();
      } catch (err) {
        console.error(err);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $("#login-form").addEventListener("submit", function (e) {
    ...
    if (pass === site.settings.adminPass) {
